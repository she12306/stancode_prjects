...
def get_best_pixel(pixels):
    ...
    dist_d = {}
    for pixel in pixels:
        dist_d[pixel] = get_pixel_dist(pixel, red, green, blue)
    best_pixel = min(dist_d, key=lambda ele: dist_d[ele])
    # Or: best_pixel = min(dist_d.items(), key=lambda ele: ele[1])[0]
    return best_pixel
...
def main():
    ...
    """
    args = sys.argv[1:] 
    args = ['hoover'] -->is a lst, args[0]-->'hoover'
    args = sys.argv[1] 
    args = 'hoover'   -->is a str, args[0]-->'h'
    Therefore, we must use [1:] rather than [1]
    ~~ substr return a lst, index return a value ~~
    """
